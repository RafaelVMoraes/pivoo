...
const sanitizeCssColor = (color: string): string | null => {
  ...
  // Allow: hex colors, rgb/rgba, hsl/hsla, named colors (letters only)
  const validColorPattern = /^(#[0-9A-Fa-f]{3,8}|rgba?\([0-9,.\s%]+\)|hsla?\([0-9,.\s%deg]+\)|[a-zA-Z]+)$/;
  
  if (!validColorPattern.test(trimmed)) {
